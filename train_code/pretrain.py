...
os.environ["CUDA_VISIBLE_DEVICES"]="0"# встановлення використання першої GPU


def arg_parser():# функція для парсингу аргументів командного рядка
...
def train(args):# основна функція тренування

    input_photo = tf.placeholder(tf.float32, [args.batch_size, args.patch_size, args.patch_size, 3])
    output = network.unet_generator(input_photo)  # вихід з генератора мережі U-Net
    recon_loss = tf.reduce_mean(tf.losses.absolute_difference(input_photo, output))  # обчислення втрати як середнього абсолютного розходження

    all_vars = tf.trainable_variables()
    gene_vars = [var for var in all_vars if 'gene' in var.name]  # вибірка змінних генератора для оптимізації
    ...
    with tf.device('/device:GPU:0'):# примусове використання GPU

        sess.run(tf.global_variables_initializer())
        face_photo_dir = 'dataset/photo_face'# завантаження списку зображень
        ...
        for total_iter in tqdm(range(args.total_iter)):
            # чергування пакетів з різних наборів даних
            ...
            if np.mod(total_iter+1, 50) == 0:
                ...
                if np.mod(total_iter+1, 500 ) == 0:# збереження моделі та генерація результатів
